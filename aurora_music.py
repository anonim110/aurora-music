...
Бэкенд: Python — YouTube (yt-dlp, полные песни) + Audius (открытый API).

Особенность: при закрытии окна музыка продолжает играть, а сверху экрана
появляется «хвостик» — при наведении он разворачивается и показывает трек,
клик по нему возвращает приложение.
